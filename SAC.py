...
TEST_EPISODES = 30  # total number of episodes for training
...
number_trajs = 1
config = {
    "method": 'our' if modify else 'dagger',
    "expert_traj": exp_trajs,
    "aggregate_traj": number_trajs*dagger_itr,
    "q_thresh": q_thresh if modify else 0
}
wandb.init(project="Pendulum", entity="mingjiali", config=config)
...
if __name__ == '__main__':
    ...
    reward_list = []
    steps = 0
    for i in range(exp_trajs):
        state = env.reset().astype(np.float32)
        episode_reward = 0
        # while True:
        for step in range(MAX_STEPS):
            # env.render()
            a = agent.policy_net.get_action(state, greedy=True)
            ...
            if done:
                ...
                state = state_.astype(np.float32)
        # tf.summary.scalar('ep_rew', data=episode_reward, step=steps)
    obs_all = np.zeros((0, state_dim))
    actions_all = np.zeros((0, action_dim))
    for ob, act in zip(ob_list, action_list):
        obs_all = np.concatenate([obs_all, np.reshape(ob, [1, state_dim])], axis=0)
        actions_all = np.concatenate([actions_all, np.reshape(act, [1, action_dim])], axis=0)

    dagger_model = tf.keras.Sequential([layers.Dense(64, activation='relu', input_shape=[state_dim]),
                                        layers.Dense(128, activation='relu'),
                                        layers.Dense(action_dim)])
    dagger_model.compile(optimizer='adam',
                         loss='mse',
                         metrics=['mae', 'mse'],
                         run_eagerly=False)
    dagger_model.fit(obs_all, actions_all,
                     batch_size=32,
                     shuffle=True)
    # #run dagger
    # state = env.reset().astype(np.float32)
    ...
    for itr in range(dagger_itr):
        ...
        for i in range(number_trajs):
            ...
            for step in range(MAX_STEPS):
                # env.render()
                a_expert = agent.policy_net.get_action(state, greedy=True)
                a = dagger_model.predict(state.reshape(-1, state_dim))[0]
                # state_, r, done, info = env.step(a)
                if modify:
                    q_input_1 = tf.concat([state.reshape(1, -1), a.reshape(1, -1)], 1)
                    q_input_2 = tf.concat([state.reshape(1, -1), a_expert.reshape(1, -1)], 1)
                    q_value_1 = tf.minimum(agent.soft_q_net1(q_input_1), agent.soft_q_net2(q_input_1))
                    q_value_2 = tf.minimum(agent.soft_q_net1(q_input_2), agent.soft_q_net2(q_input_2))
                    q_diff = q_value_2 - q_value_1
                    if q_diff > q_thresh:
                        ob_list.append(state)
                        action_list.append(a_expert)
                else:
                    ob_list.append(state)
                    action_list.append(a_expert)
                state_, r, done, info = env.step(a)
                ...
                if done:
                    ...
                    state = state_.astype(np.float32)
            print('traj', i, episode_reward)
            wandb.log({'steps': steps, 'ep_rew': episode_reward})
    ...
    if args.test:
        ...
        for episode in range(TEST_EPISODES):
            ...
            for step in range(MAX_STEPS):
                ...
                action = dagger_model.predict(state.reshape(-1, state_dim))[0]
        ...
        print('{}次测试均值{:.4f}, 标准差{:.4f}'.format(TEST_EPISODES, np.mean(test_result), np.std(test_result)))
