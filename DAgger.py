...
wandb.init(project="Breakout", entity="mingjiali", config=config)
...
print(alg_name)
# ####################  hyper parameters  ####################
if env_id in ['CartPole-v0', 'Pendulum-v0', 'MountainCar-v0', 'Acrobot-v1']:
    print(env_id)
    qnet_type = 'MLP'
    ...
    print(env_id)
    qnet_type = 'CNN'
    number_timesteps = int(1e4)  # total number of time steps to train on, 1e6
...
if __name__ == '__main__':
    ...
    # generate expert data and initialize
    ...
    for i in range(exp_trajs):
        ...
        while True:
            ...
            o_, r, done, info = env.step(a)
            ob_list.append(o)
            action_list.append(tf.one_hot(a, out_dim))
    ...
    obs_all = np.zeros((0, in_dim[0]))
    actions_all = np.zeros((0, out_dim))
    for ob, act in zip(ob_list, action_list):
        obs_all = np.concatenate([obs_all, np.reshape(ob, [1, in_dim[0]])], axis=0)
        actions_all = np.concatenate([actions_all, np.reshape(act, [1, out_dim])], axis=0)

    dagger_model = tf.keras.Sequential([layers.Dense(32, activation='relu', input_shape=in_dim),
        layers.Dense(32, activation='relu'), layers.Dense(out_dim, activation='softmax')])
    ...
    dagger_model.fit(obs_all, actions_all,
              batch_size=32,
              shuffle=True)
    # print(dagger_model.predict(obs_all))
    # run dagger
    for itr in range(dagger_itr):
        ...
        for i in range(number_trajs):
            ...
            while True:
                # env.render()
                a = dagger_model.predict(o.reshape(-1, in_dim[0]))[0].argmax()
                a_expert = dqn.get_action(o)
                o_, r, done, info = env.step(a)
                if modify:
                    obv = np.expand_dims(o, 0).astype('float32') * ob_scale
                    q_values = dqn._qvalues_func(obv).numpy()[0]
                    q_diff = q_values[a_expert] - q_values[a]
                    # print(q_values, q_diff)
                    if q_diff > q_thresh:
                        ob_list.append(o)
                        action_list.append(tf.one_hot(a_expert, out_dim))
                else:
                    ob_list.append(o)
                    action_list.append(tf.one_hot(a_expert, out_dim))
                reward_list.append(r)
